...
void MeTa::setPIN(string pin) 
{
  _at->chat("+CPIN=\"" + pin + "\"");
}

string MeTa::getPINStatus() 
{
  Parser p(_at->chat("+CPIN?", "+CPIN:"));
  return p.parseString();
}

void MeTa::setPhonebook(string phonebookName) 
{
...
vector<string> MeTa::getSupportedCharSets() 
{
  Parser p(_at->chat("+CSCS=?", "+CSCS:"));
  return p.parseStringList();
}
    
string MeTa::getCurrentCharSet() 
{
  if (_lastCharSet == "")
  {
    Parser p(_at->chat("+CSCS?", "+CSCS:"));
...
void MeTa::setCharSet(string charSetName) 
{
  _at->chat("+CSCS=\"" + charSetName + "\"");
  _lastCharSet = "";
}

string MeTa::getExtendedErrorReport() 
{
  return _at->chat("+CEER", "+CEER:");
}

void MeTa::dial(string number) 
{
  _at->chat("D" + number + ";");
}
...
vector<OPInfo> MeTa::getAvailableOPInfo() 
{
...
vector<string> MeTa::getFacilityLockCapabilities() 
{
...
vector<PWInfo> MeTa::getPasswords() 
{
...
vector<string> MeTa::getPhoneBookStrings() 
{
...
string MeTa::getServiceCentreAddress() 
{
  Parser p(_at->chat("+CSCA?", "+CSCA:"));
  return p.parseString();
}

void MeTa::setServiceCentreAddress(string sca) 
{
  int type;
...
vector<string> MeTa::getSMSStoreNames() 
{
  Parser p(_at->chat("+CPMS=?", "+CPMS:"));
...
SMSStoreRef MeTa::getSMSStore(string storeName) 
{
